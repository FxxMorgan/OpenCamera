# build_vcam.ps1 — Compila el filtro de cámara virtual DirectShow (.ax)
# Uso: .\build_vcam.ps1

$ErrorActionPreference = "Stop"

$CMAKE = "A:\msys64\mingw64\bin\cmake.exe"
$SRC   = "$PSScriptRoot\vcam_filter"
$BUILD = "$PSScriptRoot\vcam_filter\build"

Write-Host "=== Cámara Libre — Build Filtro VCam DirectShow ===" -ForegroundColor Cyan

# Configure
Write-Host "`n[1/2] Configurando CMake..." -ForegroundColor Yellow
& $CMAKE -S $SRC -B $BUILD `
    -G "MinGW Makefiles" `
    -DCMAKE_CXX_COMPILER="A:\msys64\mingw64\bin\g++.exe" `
    -DCMAKE_BUILD_TYPE=Release

if ($LASTEXITCODE -ne 0) { Write-Host "ERROR en configuración" -ForegroundColor Red; exit 1 }

# Build
Write-Host "`n[2/2] Compilando..." -ForegroundColor Yellow
& $CMAKE --build $BUILD --config Release -j4

if ($LASTEXITCODE -ne 0) {
    Write-Host "ERROR en compilación" -ForegroundColor Red; exit 1
}

Write-Host "`n✓ Filtro en: $PSScriptRoot\vcam_filter\build\libCameraLibreVCam.ax" -ForegroundColor Green
